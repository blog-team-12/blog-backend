...
func (r *UserGormRepository) GetByUsername(ctx context.Context, username string) (*entity.User, error) {
	var user entity.User
...
func (r *UserGormRepository) GetUserList(ctx context.Context, page, pageSize int) ([]*entity.User, int64, error) {
...
func (r *UserGormRepository) GetActiveUsers(ctx context.Context) ([]*entity.User, error) {
	var users []*entity.User
	err := r.db.WithContext(ctx).Where("freeze = ?", false).Find(&users).Error
	return users, err
}

// ValidateUser 验证用户登录
func (r *UserGormRepository) ValidateUser(ctx context.Context, username, password string) (*entity.User, error) {
...
func (r *UserGormRepository) UpdateLastLogin(ctx context.Context, id uint) error {
	return r.db.WithContext(ctx).Model(&entity.User{}).Where("id = ?", id).Update("updated_at", "NOW()").Error
}
