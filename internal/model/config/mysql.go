...
type Mysql struct {
	...
	Config       string `json:"configs" yaml:"configs"`               // 数据库连接的配置参数，如驱动、字符集等
...
func (m Mysql) Dsn() string {
	return m.Username + ":" + m.Password +
		"@tcp(" + m.Host + ":" + strconv.Itoa(m.Port) + ")/" + m.DBName +
		"?" + m.Config
	// 如： user:password@tcp(localhost:3306)/dbname
	//	    ?charset=utf8mb4&parseTime=True&loc=Local
}

// LogLevel 设定gorm框架内的日志等级
