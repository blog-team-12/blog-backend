...
func GetRoleID(c *gin.Context) apptype.RoleID {
	// 首先尝试从Context中获取"claims"
	if claims, exists := c.Get("claims"); !exists {
		// 如果不存在，则重新解析Access Token
		if cl, err := GetClaims(c); err != nil {
...
path（字符串）：
	...
	如果设置为 /api，则 Cookie 只对 /api 开头的路径生效.
...
domain（字符串）：
	...
	如果 domain = "example.com"，Cookie 适用于 example.com 及其子域名（如 api.example.com）。
...
//		- 2.生产环境 ： api.example.com:443 → 提取出 api.example.com
...
//			输入 api.example.com:443 → 输出 host = "api.example.com", port = "443"
